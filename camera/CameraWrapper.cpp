        ...
        const struct hw_module_t **pHmi)
{
    ...
    uint8_t *han;
    han = (uint8_t *) dlsym(handle, "_ZN7android11NvOmxCamera23getCameraSensorSettingsEP15NvxComponentRec");
    if (han) {
        uint8_t *focusCheck = han + 0x2BC;
        ALOGV("Focus check is currently %" PRIu8 "\n", *focusCheck);
        //if (*focusCheck == 0xD0)
        //    *focusCheck = 0xD1;
    }
...
static int check_vendor_module()
{
    ...
    rv = load("camera", "/vendor/lib/hw/camera.tegra3.so", (const hw_module_t**)&gVendorModule);
    if (rv) {
        ...
        int32_t msg_type)
{
    ...
    msg_type &= ~CAMERA_MSG_FOCUS_MOVE;
...
static int camera_auto_focus(struct camera_device *device)
{
    ...
    //ioctl(wrapper->fd, OV5640_IOCTL_AF_TRIGGER, 1);
#endif
        ...
        const char *parameters)
{
    ...
    params.set("nv-stereo-mode-values", "");
...
#ifdef WRAP_FOCUS
        camera_device->fd = open("/dev/ov5640", 0);
#endif
    }
